...
export function DungeonViewport({ scene, threats, isCombatActive, onThreatClick }: DungeonViewportProps) {
  const aliveThreats = threats.filter(threat => threat.isAlive);

  return (
    <div className="relative flex-1 min-h-[240px] rounded-lg overflow-hidden border border-slate-800 bg-slate-900">
      <img
        src={scene.imagePath}
        alt={scene.location}
          ...
          {aliveThreats.map(threat => {
            ...
            return (
              <button
                ...
                className="flex flex-col items-center gap-1 bg-black/60 hover:bg-black/80 border border-red-800 rounded px-2 py-1 transition-colors"
              >
